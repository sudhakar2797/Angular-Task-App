...
export class UserpanelComponent implements OnInit {
  ...
  completed = 0;
  heading = 'Dashboard';
  subheading = 'dashboard here you can find all your task details';
  ...
  ngOnInit() {
    this.http.get(this.dataProvider.taskDetails)
      .subscribe(
        (res) => {
          this.response = res;
          this.dataProvider.data = res;
          this.response.forEach(item => {
            if (item.assigned_to === localStorage.getItem('loginId')) {
              if (item.task_status === 'assigned') {
                this.assigned++;
              } else if (item.task_status === 'completed') {
                this.completed++;
              }
              this.total++;
              this.filtRes.push(item);
            }
          });
        });
  }
